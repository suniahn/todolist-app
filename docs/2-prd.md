...
| 버전 | 날짜 | 작성자 | 변경 내용 |
|------|------|--------|-----------|
| 1.3 | 2026-05-14 | sunny | 다크모드(UI 모드 전환) 1차 구현으로 반영, API·화면 목록·백로그 업데이트 |
| 1.2 | 2026-05-14 | sunny | 카테고리 관리 기능(UC-09, UC-10) 1차 구현으로 범위 조정, API 엔드포인트 및 화면 목록 업데이트 |
...
| 릴리즈 | 목표 | 기간 |
|--------|------|------|
| **1차 (MVP)** | 인증 + 핵심 할일 관리 + 카테고리 관리 + UI 모드 전환 | 3일 |
| **2차** | 프로필 관리, OAuth, 다국어 | 추후 |
...
| UC-08 | 할일 목록 조회·필터 | ✅ | | |
| UC-09 | 커스텀 카테고리 추가 | ✅ | | |
| UC-10 | 커스텀 카테고리 삭제 | ✅ | | 할일이 있는 카테고리 삭제 차단 (CATEGORY_IN_USE) |
| — | 로그아웃 | ✅ | | |
| — | UI 모드 전환 (라이트/다크) | ✅ | | localStorage에 설정 유지, 재로그인 시에도 적용 |
| — | 회원탈퇴 | | ✅ | 탈퇴 시 데이터 즉시 삭제 |
...
#### UC-09 커스텀 카테고리 추가
- **입력:** 카테고리 이름 (필수, 최대 50자)
- **처리:**
  - 동일 이름의 카테고리가 해당 사용자에게 이미 존재하면 `VALIDATION_ERROR` (400)
  - 기본 카테고리(`is_default=TRUE`)와 이름이 같아도 사용자 카테고리로 별도 생성 가능
- **유효성 검사:**
  - 이름: 필수, 최대 50자

#### UC-10 커스텀 카테고리 수정
- **입력:** 카테고리 ID (경로), 새 이름 (필수, 최대 50자)
- **처리:**
  - 기본 카테고리(`is_default=TRUE`) 수정 불가 → `VALIDATION_ERROR` (400)
  - 본인 소유 카테고리만 수정 가능 → 타인 소유 시 `CATEGORY_NOT_FOUND` (404)
  - 동일 이름 중복 시 `VALIDATION_ERROR` (400)

#### UC-11 커스텀 카테고리 삭제
- **처리:**
  - 기본 카테고리 삭제 불가 → `VALIDATION_ERROR` (400)
  - 본인 소유 카테고리만 삭제 가능
  - 해당 카테고리에 할일이 존재하면 삭제 차단 → `CATEGORY_IN_USE` (409)
  - 삭제는 물리 삭제
...
| Method | Path | 설명 | 인증 |
|--------|------|------|:----:|
| GET | `/categories` | 사용 가능한 카테고리 목록 (기본 + 본인 소유) | ✅ |
| POST | `/categories` | 사용자 카테고리 생성 | ✅ |
| PUT | `/categories/:id` | 사용자 카테고리 수정 | ✅ |
| DELETE | `/categories/:id` | 사용자 카테고리 삭제 | ✅ |
...
- UI 모드 전환: 헤더 토글 버튼으로 라이트/다크 모드 전환, `localStorage`에 저장하여 재방문 시 설정 유지
